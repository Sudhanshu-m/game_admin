...
const trackStudentActivity = async (studentEmail: string) => {
  try {
    ...
    const activityLog = await kv.get(activityKey) || { dates: [], currentStreak: 0, longestStreak: 0 };
    ...
    if (!activityLog.dates.includes(todayStr)) {
      ...
      activityLog.longestStreak = Math.max(activityLog.longestStreak || 0, currentStreak);
      activityLog.lastActivityDate = todayStr;
      
      // Keep only last 365 days of activity
      const oneYearAgo = new Date();
      oneYearAgo.setFullYear(oneYearAgo.getFullYear() - 1);
      activityLog.dates = activityLog.dates.filter(date => new Date(date) >= oneYearAgo);
...
const getStudentStreak = async (studentEmail: string) => {
  try {
    ...
    return { dates: [], currentStreak: 0, longestStreak: 0, lastActivityDate: null };
  }
};
    ...
    for (const grade of grades) {
      if (grade.studentEmail) {
        ...
        const updatedTasks = studentTasks.map(task => {
          ...
          const partialMatch = taskTitle.includes(assignmentName) || assignmentName.includes(taskTitle);
          
          if ((exactMatch || partialMatch) && !task.completed) {
            console.log(`    ✓ MATCH FOUND! Marking task "${task.title}" as completed`);
        ...
        if (taskMarkedComplete) {
          ...
          console.log('✗ No matching task found for assignment:', grade.assignment);
        }
...
app.post("/make-server-2fad19e1/student/signup", async (c) => {
  try {
    const { name, username, email, rollNumber, batch, password } = await c.req.json();

    console.log('Student signup attempt:', { name, username, email, rollNumber, batch });
    ...
    if (existingUsername) {
      ...
      return c.json({ error: 'Username already taken. Please choose another one.' }, 400);
    }
    ...
    console.log('Student signup complete:', { id: data.user.id, name, username, email, rollNumber, batch });
...
app.post("/make-server-2fad19e1/student/profile/update", async (c) => {
  try {
    ...
    const profileData = await c.req.json();
    console.log('Updating student profile for:', user.email, 'with data:', profileData);
...
app.get("/make-server-2fad19e1/teacher/task-stats", async (c) => {
  try {
    ...
    console.log('Loaded tasks data for', studentTasksMap.size, 'students (OPTIMIZED)');
...
app.post("/make-server-2fad19e1/teacher/task-students", async (c) => {
  try {
    ...
    console.log('Students in class:', assignedClassStudents.length + manualClassStudents.length);
    ...
    console.log('Loaded grades for', Object.keys(taskGrades).length, 'students');
    ...
    for (const assignment of assignedClassStudents) {
      ...
      console.log(`Processing ${studentEmail}: hasGrade=${hasGrade}, grade=${taskGrades[studentEmail]}`);
    ...
    for (const student of manualClassStudents) {
      ...
      console.log(`Processing manual ${student.email}: hasGrade=${hasGrade}, grade=${taskGrades[student.email]}`);
    ...
    return c.json({ error: 'Failed to get task students: ' + error.message }, 500);
  }
});
...
app.get("/make-server-2fad19e1/student/daily-quest", async (c) => {
  try {
    ...
    return c.json({ error: 'Failed to get daily quest: ' + error.message }, 500);
  }
});
...
app.post("/make-server-2fad19e1/student/daily-quest/seen", async (c) => {
  try {
    ...
    return c.json({ error: 'Failed to mark quest as seen: ' + error.message }, 500);
  }
});
...
app.post("/make-server-2fad19e1/admin/broadcast", async (c) => {
  try {
    ...
    return c.json({ error: 'Failed to create broadcast: ' + error.message }, 500);
  }
});
...
app.delete("/make-server-2fad19e1/admin/broadcast", async (c) => {
  try {
    ...
    return c.json({ error: 'Failed to delete broadcast: ' + error.message }, 500);
  }
});
...
app.post("/make-server-2fad19e1/teacher/daily-quest", async (c) => {
  try {
    ...
    return c.json({ error: 'Failed to create daily quest: ' + error.message }, 500);
  }
});
...
app.get("/make-server-2fad19e1/teacher/student-tasks/:email", async (c) => {
  try {
    ...
    return c.json({ error: 'Failed to get student tasks: ' + error.message }, 500);
  }
});
...
app.get("/make-server-2fad19e1/teacher/student-streak/:email", async (c) => {
  try {
    ...
    return c.json({ error: 'Failed to get student streak: ' + error.message }, 500);
  }
});
...
app.post("/make-server-2fad19e1/teacher/students-batch-data", async (c) => {
  try {
    ...
    console.log('Loaded data - Tasks:', studentTasksMap.size, 'Activity:', studentActivityMap.size);
    ...
    for (const email of studentEmails) {
      ...
      if (activity.dates && activity.dates.length > 0) {
        const sortedDates = activity.dates.sort((a, b) => new Date(b).getTime() - new Date(a).getTime());
        let currentStreak = 0;
    ...
    return c.json({ error: 'Failed to get batch student data: ' + error.message }, 500);
  }
});
...
app.get("/make-server-2fad19e1/teacher/task-grades/:taskId", async (c) => {
  try {
    ...
    return c.json({ error: 'Failed to get task grades: ' + error.message }, 500);
  }
});
...
app.post("/make-server-2fad19e1/teacher/task-grade", async (c) => {
  try {
    ...
    const updatedTasks = studentTasks.map(t => {
      if (t.id === taskId) {
        console.log(`  ✓ Found task "${t.title}" - marking as completed with grade ${grade}`);
        taskUpdated = true;
        return {
          ...t,
          completed: true,
...
app.post("/make-server-2fad19e1/teacher/quest", async (c) => {
  try {
    ...
    console.log(`Creating notifications for ${classStudents.length} students in class ${class_id}`);
...
app.get("/make-server-2fad19e1/quest", async (c) => {
  try {
    ...
    if (quest && studentClassId && quest.classId && quest.classId !== studentClassId) {
      return c.json({ quest: null });
    }
